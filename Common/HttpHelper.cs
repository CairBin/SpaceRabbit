...
namespace Commons
{
    ...
    {
        public static async Task SaveToFileAsync(this HttpResponseMessage msg, string file, CancellationToken cancellationToken = default)
        {
            if(msg.IsSuccessStatusCode == false)
            {
                throw new ArgumentException($"StatusCode of HttpResponseMessage is {msg.StatusCode}", nameof(msg));
            }

            using FileStream fs = new FileStream(file, FileMode.Create);
            await msg.Content.CopyToAsync(fs, cancellationToken);
        }
